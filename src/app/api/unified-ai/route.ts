...
async function checkOllama(): Promise<boolean> {
  try {
    const response = await fetch(`${process.env.OLLAMA_HOST || 'https://eleven-boxes-win.loca.lt'}/api/tags`, {
...
async function callOllama(message: string) {
  const host = process.env.OLLAMA_HOST || 'https://eleven-boxes-win.loca.lt'
