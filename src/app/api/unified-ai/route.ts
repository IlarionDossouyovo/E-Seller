...
async function checkOllama(): Promise<boolean> {
  try {
    const response = await fetch(`${process.env.OLLAMA_HOST || 'https://nice-banks-rule.loca.lt'}/api/tags`, {
...
async function callOllama(message: string) {
  const host = process.env.OLLAMA_HOST || 'https://nice-banks-rule.loca.lt'
