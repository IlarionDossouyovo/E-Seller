export const translation = {
  // Navigation
  nav: {
    dashboard: 'Dashboard',
    products: 'Products',
    branding: 'Branding',
    ads: 'Ads',
    analytics: 'Analytics',
    assistant: 'Assistant',
    suppliers: 'Suppliers',
    orders: 'Orders',
    payments: 'Payments',
    settings: 'Settings',
    logout: 'Logout',
  },
  
  // Dashboard
  dashboard: {
    title: 'Dashboard',
    revenue: 'Revenue',
    products: 'Products',
    roas: 'ROAS',
    users: 'Users',
    welcome: 'Welcome back',
  },
  
  // Common
  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    search: 'Search',
    filter: 'Filter',
    export: 'Export',
    import: 'Import',
    loading: 'Loading...',
    noResults: 'No results found',
    confirm: 'Confirm',
    back: 'Back',
    next: 'Next',
    previous: 'Previous',
    submit: 'Submit',
    close: 'Close',
  },
  
  // AI Agents
  ai: {
    title: 'AI Dashboard',
    productIntelligence: 'Product Intelligence',
    supplierEngine: 'Supplier Engine',
    brandingGenerator: 'Branding Generator',
    adsGenerator: 'Ads Generator',
    positioningEngine: 'Positioning Engine',
    marketAnalytics: 'Market Analytics',
    businessAssistant: 'Business Assistant',
    recommendations: 'Recommendations',
  },
  
  // Status
  status: {
    active: 'Active',
    inactive: 'Inactive',
    pending: 'Pending',
    error: 'Error',
    success: 'Success',
    online: 'Online',
    offline: 'Offline',
  },
  
  // Footer
  footer: {
    allRightsReserved: 'All rights reserved',
    privacyPolicy: 'Privacy Policy',
    termsOfService: 'Terms of Service',
    contact: 'Contact',
  },
  
  // Auth
  auth: {
    login: 'Login',
    register: 'Register',
    logout: 'Logout',
    email: 'Email',
    password: 'Password',
    forgotPassword: 'Forgot password?',
    rememberMe: 'Remember me',
  },
  
  // Settings
  settings: {
    title: 'Settings',
    language: 'Language',
    currency: 'Currency',
    timezone: 'Timezone',
    notifications: 'Notifications',
    security: 'Security',
    profile: 'Profile',
    account: 'Account',
  },

  // Settings Page
  settingsPage: {
    title: 'Settings',
    subtitle: 'Configure your platform',
    general: 'General',
    store: 'Store',
    payments: 'Payments',
    notifications: 'Notifications',
    security: 'Security',
    appearance: 'Appearance',
    email: 'Email',
    api: 'API',
    saveChanges: 'Save Changes',
    saved: 'Saved!',
  },

  // General Settings
  general: {
    storeName: 'Store Name',
    storeEmail: 'Store Email',
    timezone: 'Timezone',
    defaultLanguage: 'Default Language',
    save: 'Save',
  },

  // Store Settings
  store: {
    title: 'Store Configuration',
    maintenanceMode: 'Maintenance Mode',
    maintenanceModeDesc: 'Disable store temporarily',
    guestCheckout: 'Guest Checkout',
    guestCheckoutDesc: 'Allow checkout without account',
    currencySelector: 'Currency Selector',
    currencySelectorDesc: 'Show currency switcher',
  },

  // Payment Settings
  payments: {
    title: 'Payment Settings',
    currency: 'Currency',
    taxRate: 'Tax Rate (%)',
    taxIncluded: 'Tax Included',
    taxIncludedDesc: 'Prices include tax',
  },

  // Notifications
  notifications: {
    title: 'Notifications',
    newOrder: 'New Order',
    newOrderDesc: 'Email when order placed',
    lowStock: 'Low Stock',
    lowStockDesc: 'Alert when product low',
    customerRegistration: 'Customer Registration',
    customerRegistrationDesc: 'Notify new customer',
    paymentReceived: 'Payment Received',
    paymentReceivedDesc: 'Confirm payment',
  },

  // Security
  security: {
    title: 'Security',
    twoFactorAuth: 'Two-Factor Auth',
    twoFactorAuthDesc: 'Require 2FA for admin',
    ipWhitelist: 'IP Whitelist',
    ipWhitelistDesc: 'Restrict access by IP',
    sessionTimeout: 'Session Timeout',
    sessionTimeoutDesc: 'Auto logout after 30min',
  },

  // Appearance
  appearance: {
    title: 'Appearance',
    theme: 'Theme',
    darkMode: 'Dark Mode',
    lightMode: 'Light Mode',
  },

  // Email Settings
  email: {
    title: 'Email Configuration',
    smtpHost: 'SMTP Host',
    smtpPort: 'SMTP Port',
    smtpUser: 'SMTP Username',
    smtpPassword: 'SMTP Password',
    fromEmail: 'From Email',
    fromName: 'From Name',
  },

  // API Settings
  api: {
    title: 'API Keys',
    productionKey: 'Production Key',
    developmentKey: 'Development Key',
    generateNewKey: 'Generate New Key',
    view: 'View',
  },
  
  // Languages
  languages: {
    en: 'English',
    fr: 'French',
    es: 'Spanish',
    de: 'German',
    zh: 'Chinese',
    ja: 'Japanese',
    pt: 'Portuguese',
    ar: 'Arabic',
  },

  // Email Marketing
  emailMarketing: {
    title: 'Email Marketing',
    subtitle: 'Create and manage email campaigns',
    newCampaign: 'New Campaign',
    editCampaign: 'Edit Campaign',
    campaignName: 'Campaign Name',
    campaignNamePlaceholder: 'e.g., Summer Sale',
    subjectLine: 'Subject Line',
    subjectPlaceholder: 'e.g., Big Sale Inside!',
    status: 'Status',
    createCampaign: 'Create Campaign',
    saveChanges: 'Save Changes',
    cancel: 'Cancel',
    totalSubscribers: 'Total Subscribers',
    avgOpenRate: 'Avg Open Rate',
    avgClickRate: 'Avg Click Rate',
    sentThisMonth: 'Sent This Month',
    campaigns: 'Campaigns',
    templates: 'Templates',
    automation: 'Automation',
    newsletter: 'Newsletter',
    searchPlaceholder: 'Search campaigns...',
    all: 'All',
    draft: 'Draft',
    scheduled: 'Scheduled',
    sending: 'Sending',
    sent: 'Sent',
    failed: 'Failed',
    notScheduled: 'Not scheduled',
    opened: 'Opened',
    clicked: 'Clicked',
    send: 'Send',
    edit: 'Edit',
    view: 'View',
    duplicate: 'Duplicate',
    delete: 'Delete',
    activate: 'Activate',
    pause: 'Pause',
    stats: 'Statistics',
    confirmDelete: 'Are you sure you want to delete this campaign?',
  },

  // Funnel Builder
  funnelBuilder: {
    title: 'Funnel Builder',
    subtitle: 'Create automated marketing funnels',
    newFunnel: 'New Funnel',
    totalFunnels: 'Total Funnels',
    active: 'Active',
    conversions: 'Conversions',
    revenue: 'Revenue',
    steps: 'steps',
    edit: 'Edit',
    delete: 'Delete',
    duplicate: 'Duplicate',
    pause: 'Pause',
    activate: 'Activate',
    settings: 'Settings',
    save: 'Save',
    cancel: 'Cancel',
    createFunnel: 'Create Funnel',
    funnelName: 'Funnel Name',
    funnelNamePlaceholder: 'e.g., Summer Sale Funnel',
    funnelType: 'Funnel Type',
    productLaunch: 'Product Launch',
    leadMagnet: 'Lead Magnet',
    webinar: 'Webinar',
    upsell: 'Upsell',
    custom: 'Custom',
  },

  // Super Admin
  superAdmin: {
    title: 'Super Admin',
    subtitle: 'Platform-wide management',
    totalTenants: 'Total Tenants',
    activeUsers: 'Active Users',
    monthlyRevenue: 'Monthly Revenue',
    systemHealth: 'System Health',
    optimal: 'Optimal',
    tenants: 'Tenants',
    search: 'Search...',
    all: 'All',
    active: 'Active',
    pending: 'Pending',
    suspended: 'Suspended',
    name: 'Name',
    plan: 'Plan',
    users: 'Users',
    revenue: 'Revenue',
    status: 'Status',
    recentActivity: 'Recent Activity',
    newTenantRegistered: 'New tenant registered',
    paid: 'paid',
    exceededUsageLimit: 'exceeded usage limit',
    adminLogin: 'Admin login from',
    quickActions: 'Quick Actions',
    addTenant: 'Add Tenant',
    viewReports: 'View Reports',
    systemSettings: 'System Settings',
    viewAllLogs: 'View All Logs',
  },

  // Reports
  reports: {
    title: 'Reports',
    subtitle: 'Generate and download business reports',
    search: 'Search reports...',
    allTypes: 'All Types',
    sales: 'Sales',
    inventory: 'Inventory',
    customers: 'Customers',
    tax: 'Tax',
    financial: 'Financial',
    shipping: 'Shipping',
    generateReport: 'Generate Report',
    view: 'View',
    download: 'Download',
    monthlySalesReport: 'Monthly Sales Report',
    inventoryStatus: 'Inventory Status',
    customerAnalytics: 'Customer Analytics',
    taxSummary: 'Tax Summary',
    profitLoss: 'Profit & Loss',
    shippingReport: 'Shipping Report',
  },

  // Environment Variables
  environment: {
    title: 'Environment Variables',
    subtitle: 'Manage API keys and sensitive configuration',
    addNewVariable: 'Add New Variable',
    keyName: 'KEY_NAME',
    value: 'value',
    custom: 'Custom',
    database: 'Database',
    payment: 'Payment',
    auth: 'Auth',
    email: 'Email',
    storage: 'Storage',
    production: 'Production',
    staging: 'Staging',
    development: 'Development',
    add: 'Add',
    securityNotice: 'Security Notice',
    securityMessage: 'Never share your production API keys. Use environment variables to keep them secure.',
    copied: 'Copied!',
    delete: 'Delete',
  },

  // Customers
  customers: {
    title: 'Customers',
    subtitle: 'Manage your customer base',
    totalCustomers: 'Total Customers',
    active: 'Active',
    vip: 'VIP',
    totalRevenue: 'Total Revenue',
    search: 'Search customers...',
    allStatus: 'All Status',
    addCustomer: 'Add Customer',
    orders: 'Orders',
    spent: 'Spent',
    last: 'Last',
    email: 'Email',
    viewOrders: 'Orders',
  },

  // Affiliate Management
  affiliate: {
    title: 'Affiliate Management',
    subtitle: 'Manage your affiliate partners',
    affiliates: 'Affiliates',
    payouts: 'Payouts',
    tracking: 'Tracking',
    totalAffiliates: 'Total Affiliates',
    active: 'Active',
    totalSales: 'Total Sales',
    totalEarnings: 'Total Earnings',
    affiliate: 'Affiliate',
    commission: 'Commission',
    sales: 'Sales',
    earnings: 'Earnings',
    joined: 'Joined',
    status: 'Status',
    amount: 'Amount',
    date: 'Date',
    paid: 'Paid',
    pending: 'Pending',
  },

  // Returns Management
  returns: {
    title: 'Returns Management',
    subtitle: 'Process return requests',
    totalRequests: 'Total Requests',
    pending: 'Pending',
    approved: 'Approved',
    refunded: 'Refunded',
    returnId: 'Return ID',
    order: 'Order',
    product: 'Product',
    reason: 'Reason',
    amount: 'Amount',
    date: 'Date',
    status: 'Status',
    actions: 'Actions',
    defective: 'Defective',
    wrongSize: 'Wrong Size',
    notAsDesc: 'Not as described',
    changedMind: 'Changed mind',
    rejected: 'Rejected',
  },

  // Suppliers Management
  suppliers: {
    title: 'Suppliers Management',
    subtitle: 'Manage your suppliers and purchase orders',
    suppliers: 'Suppliers',
    orders: 'Orders',
    analytics: 'Analytics',
    totalSuppliers: 'Total Suppliers',
    active: 'Active',
    products: 'Products',
    avgRating: 'Avg Rating',
    supplier: 'Supplier',
    country: 'Country',
    productsCount: 'Products',
    leadTime: 'Lead Time',
    rating: 'Rating',
    status: 'Status',
  },

  // Taxes Management
  taxes: {
    title: 'Taxes Management',
    subtitle: 'Configure tax rates and file returns',
    rates: 'Rates',
    filings: 'Filings',
    reports: 'Reports',
    taxRates: 'Tax Rates',
    collected: 'Collected',
    averageRate: 'Average Rate',
    dueSoon: 'Due Soon',
    taxName: 'Tax Name',
    rate: 'Rate',
    type: 'Type',
    region: 'Region',
    collectedAmount: 'Collected',
    status: 'Status',
    state: 'State',
    country: 'Country',
    period: 'Period',
    dueDate: 'Due Date',
    amountDue: 'Amount Due',
    payNow: 'Pay Now',
    paid: 'Paid',
    actions: 'Actions',
  },

  // Vendor Dashboard
  vendor: {
    title: 'Vendor Dashboard',
    revenue: 'Revenue',
    products: 'Products',
    roas: 'ROAS',
    users: 'Users',
    totalSales: 'Total Sales',
    orders: 'Orders',
    customers: 'Customers',
    rating: 'Rating',
    overview: 'Overview',
    analytics: 'Analytics',
    payouts: 'Payouts',
    recentOrders: 'Recent Orders',
    viewAll: 'View All',
    order: 'Order',
    customer: 'Customer',
    total: 'Total',
    status: 'Status',
    topProducts: 'Top Products',
    sales: 'sales',
    stock: 'stock',
    editStore: 'Edit Store',
    verified: 'Verified',
    level: 'Level',
    gold: 'Gold',
    pending: 'Pending',
    processing: 'Processing',
    shipped: 'Shipped',
    delivered: 'Delivered',
    orderId: 'Order ID',
    product: 'Product',
    quantity: 'Quantity',
    date: 'Date',
    actions: 'Actions',
    myProducts: 'My Products',
    addProduct: 'Add Product',
    inStock: 'in stock',
    outOfStock: 'Out of stock',
    edit: 'Edit',
    view: 'View',
    nextPayout: 'Next Payout',
    payoutAmount: 'Amount',
    payoutDate: 'Date',
    payoutHistory: 'Payout History',
    amount: 'Amount',
    requested: 'Requested',
    paid: 'Paid',
    viewDetails: 'View Details',
    grossSales: 'Gross Sales',
    commission: 'Commission (10%)',
    processingFee: 'Processing Fee',
    method: 'Method',
  },

  // Vendor Registration
  vendorRegistration: {
    title: 'Become a Vendor',
    subtitle: 'Join the E-Seller marketplace',
    storeInfo: 'Store Info',
    location: 'Location',
    verification: 'Verification',
    payment: 'Payment',
    storeName: 'Store Name',
    storeUrl: 'Store URL',
    email: 'Email',
    phone: 'Phone',
    continue: 'Continue',
    back: 'Back',
    submitApplication: 'Submit Application',
    storeNameRequired: 'Store name is required',
    storeUrlRequired: 'Store URL is required',
    emailRequired: 'Email is required',
    phoneRequired: 'Phone is required',
    selectCountry: 'Select Country',
    country: 'Country',
    businessAddress: 'Business Address',
    productCategory: 'Product Category',
    selectCategory: 'Select Category',
    storeDescription: 'Store Description',
    describeStore: 'Describe your store and products...',
    taxId: 'Tax ID / VAT Number',
    businessLicense: 'Business License',
    uploadLicense: 'Upload business license or registration',
    maxFileSize: 'PDF, JPG, PNG - Max 10MB',
    verificationRequired: 'Verification Required',
    verificationMessage: 'We will verify your business within 24-48 hours. You can start adding products while verification is pending.',
    paymentSettings: 'Payment Settings',
    bankName: 'Bank Name',
    accountNumber: 'Account Number',
    routingNumber: 'Routing Number',
    or: 'OR',
    paypalEmail: 'PayPal Email',
    commission: 'Commission',
    onEachSale: 'on each sale',
    freeToJoin: 'Free to Join',
    noUpfrontFees: 'No upfront fees. Only pay 10% commission on sales.',
    globalReach: 'Global Reach',
    sellTo190: 'Sell to customers in 190+ countries with localized checkout.',
    fastPayouts: 'Fast Payouts',
    getPaidWeekly: 'Get paid weekly via bank transfer or PayPal.',
    yourStoreName: 'Your Store Name',
    yourStore: 'your store',
    vendorAtStore: 'vendor@store.com',
    phoneNumber: '+1 (555) 000-0000',
    fullAddress: 'Full business address',
    taxIdPlaceholder: 'XX-XXXXXXX',
    bankExample: 'Bank of America',
    accountPlaceholder: 'XXXX XXXX XXXX',
    routingPlaceholder: 'XXXXXXX',
    paypalExample: 'paypal@vendor.com',
    applicationSubmitted: 'Application submitted! We will review your application within 24-48 hours.',
    selectPayment: 'Payment method',
  },
}

export type Translation = typeof translation